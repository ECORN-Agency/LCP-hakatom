...
Исправлено (все с юнит-тестами): **H1, H2, H3, M1, M2, M3, M4, L1, L3**.
- **L1** — `bearerMatches` на `crypto.timingSafeEqual` (SHA-256 обеих сторон),
  применён в `api.jobs.run`, `poll-themes`, `evaluate-alerts`.
- **L3** — N+1 `alertDelivery.findUnique` заменён на один `findMany` + `Set`
  по (ruleId, changeId) на магазин; 250ms-троттлинг отправок намеренно оставлен
  (лимит Resend 5/sec).

Открыто: **L2** (idempotency-ключ для поллинга — практически снят после H3),
плюс полноценный rate-limit pixel по shop+IP (требует стора Redis/БД — отдельный
заход). M3 закрыт по сути: кап размера `data` + отбраковка `occurredAt` вне ±24ч.
